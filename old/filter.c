...
void pixHDelta(uint32_t* px, uint32_t* out, int h, int w){
	int ix = 0;
	for(int i = 0; i < h; i++){
		for(int j = 0; j < w; j++){
			uint32_t p  =  px[ix];
			...
			uint8_t* pb = (uint8_t*)&p;
			uint8_t* lb = (uint8_t*)&l;
			for(int k = 0; k < 4; k++) pb[k] = (pb[k] != lb[k]) * 128;
...
void pixVDelta(uint32_t* px, uint32_t* out, int h, int w){
	int ix = 0;
	for(int i = 0; i < h; i++){
		for(int j = 0; j < w; j++){
			uint32_t p  =  px[ix];
			...
			uint8_t* pb = (uint8_t*)&p;
			uint8_t* vb = (uint8_t*)&v;
			for(int k = 0; k < 4; k++) pb[k] = (pb[k] != vb[k]) * 128;
